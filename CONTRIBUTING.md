...
运行 `make help` 可以查看维护的 Make targets。`make build` 和 `make build-bin` 会按锁定
版本生成规则集目录快照，并复用 XDG cache 中的上游 Git objects；默认质量门 `make check`
使用已有源码完成检查，不依赖该下载步骤。发布构建的 `VERSION`、`REVISION` 和容器
metadata 契约见[构建身份](docs/reference/build-info.md)。
...
不包含 Web 检查。远端 [CI](.github/workflows/ci.yml) 将 Web 静态检查、完整 Vitest
与 built SPA Playwright smoke 拆为并行 job；三者通过后才进入部署或发布阶段。
