...
Web 的“生成地区组”只在点击生成时修改分组；勾选、取消或调整工具选项不改写
已有配置。保存的是客户端原生 groups，工具选项不另外持久化。

重开时，仅在候选地区组全部匹配当前标准结构、名称无冲突、地区不重复且类型统一时，
恢复勾选、实际存在的地区和统一类型。候选按已知地区名称或精确标准正则确定；
普通其他分组不影响判断。改名、混合类型、旧别名或自定义结构等情况不认领，工具
保持未勾选并使用客户端默认选项，已有分组原样保留。

sing-box 生成仍需节点预览，以跳过没有匹配节点的地区；重开只恢复实际存在的地区，
不推测上次被跳过的选项。已有固定成员组不会自动迁移为正则组。
