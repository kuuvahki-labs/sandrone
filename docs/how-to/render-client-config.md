...
Web 的“生成地区组”在点击时生成客户端原生 groups。自定义分组保留，重开
页面时无法识别的配置会让工具选项回到默认值。sing-box 使用节点预览显示当前
匹配数并检查名称冲突，但会保存所有明确选择的地区正则；出站适配处理器在每次
渲染时删除当次没有节点的地区组，并在节点重新出现后恢复。需要自定义结构时可
直接编辑分组。
