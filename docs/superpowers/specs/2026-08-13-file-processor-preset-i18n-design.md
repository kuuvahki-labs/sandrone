...
args:
  ...
  replacements_json: '[["source","destination"]]'
```
...
新预设通过通用脚本源码和稳定的 `args.preset_id` 识别，允许用户修改 `replacements_json` 后仍保持预设身份。识别逻辑同时接受旧版带 `sandrone:file-preset=github-rule-source-rewrite` marker 的内联脚本，从而：
