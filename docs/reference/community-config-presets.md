...
| --- | --- | --- | --- |
| Mihomo | base 内置 Sniffer、TUN 与 loopback Clash API；`allow-lan: true`；`lan-allowed-ips` 只含 RFC1918 IPv4 与 `fc00::/7`；Geo 数据自动更新且间隔 24 小时；显式设置 `disable-keep-alive: true`；新建 `url-test` 分组写入 `tolerance: 50`。 | TUN 默认接管系统路由和 DNS；需要关闭时修改 base 或添加后置 processor。API 不对 LAN 暴露，但本机进程可访问，且与同机使用 `127.0.0.1:9090` 的进程冲突。默认值不回填已有文件或分组。 | [Mihomo 全局配置](https://wiki.metacubex.one/en/config/general/)、[Mihomo Sniffer](https://wiki.metacubex.one/en/config/sniff/)、[Mihomo TUN](https://wiki.metacubex.one/en/config/inbound/tun/) |
| sing-box | base 内置 sniff、DNS hijack、双栈 TUN、loopback Clash API 与 `direct`/`global` 模式规则；localhost mixed inbound 为系统 HTTP proxy 提供入口；DNS 使用 1.14 FakeIP server，普通 A/AAAA 返回 FakeIP，内置连接检测、本地、校时/NTP/STUN、iCloud 与 Xiaomi 等兼容域名走真实 DNS；新建 `urltest` 写入 `tolerance: 50`。 | TUN 默认接管系统路由和 DNS；需要关闭时修改 base 或添加后置 processor。API 不对 LAN 暴露，但本机进程可访问，且与同机使用 `127.0.0.1:9090` 的进程冲突。默认值不回填已有文件或分组。 | [sing-box Clash API](https://sing-box.sagernet.org/configuration/experimental/clash-api/)、[sing-box Sniff](https://sing-box.sagernet.org/configuration/route/rule_action/#sniff)、[sing-box FakeIP](https://sing-box.sagernet.org/configuration/dns/server/fakeip/)、[sing-box TUN](https://sing-box.sagernet.org/configuration/inbound/tun/)、[sing-box URLTest](https://sing-box.sagernet.org/configuration/outbound/urltest/) |
