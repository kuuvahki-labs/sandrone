...
| --- | --- | --- | --- |
| Mihomo | base 内置 Sniffer、TUN、APNs 真实解析/直连与 loopback Clash API；`allow-lan: true`；`lan-allowed-ips` 只含 RFC1918 IPv4 与 `fc00::/7`；Geo 数据自动更新且间隔 24 小时；显式设置 `disable-keep-alive: true`；新建 `url-test` 分组写入 `tolerance: 50`。 | TUN 默认接管系统路由和 DNS，但绕过 Apple 公布的 APNs 网段；需要关闭或让受限网络中的 APNs 走代理时修改 base 或添加后置 processor。API 不对 LAN 暴露，但本机进程可访问，且与同机使用 `127.0.0.1:9090` 的进程冲突。默认值不回填已有文件或分组。 | [Mihomo 全局配置](https://wiki.metacubex.one/en/config/general/)、[Mihomo DNS](https://wiki.metacubex.one/en/config/dns/)、[Mihomo Sniffer](https://wiki.metacubex.one/en/config/sniff/)、[Mihomo TUN](https://wiki.metacubex.one/en/config/inbound/tun/)、[Apple APNs network requirements](https://support.apple.com/102266) |
| sing-box | base 内置 LAN 来源守卫、sniff、本地域名解析、DNS hijack、双栈 TUN、APNs 真实解析/直连、loopback Clash API 与 `direct`/`global` 模式规则；mixed inbound 监听 `0.0.0.0:2080`，仅接受 loopback/RFC1918 来源。DNS 使用 1.14 FakeIP server，保留 IPv4/IPv6 范围但以 `ipv4_only` 默认关闭 IPv6 解析；系统 hosts、邻居主机、mDNS、APNs 与稳定本地域名走 `dns-local`；新建 `urltest` 写入 `tolerance: 50`。 | TUN 默认接管系统路由和 DNS，但绕过 Apple 公布的 APNs 网段，且不额外启用平台 HTTP proxy；需要关闭或让受限网络中的 APNs 走代理时修改 base 或添加后置 processor。LAN mixed 未配置认证，只应在受信任网络使用；Clash API 仍只监听本机，且会与同机 `127.0.0.1:9090` 冲突。connectivity/captive portal 由平台客户端负责。默认值不回填已有文件或分组。 | [sing-box Clash API](https://sing-box.sagernet.org/configuration/experimental/clash-api/)、[sing-box Local DNS](https://sing-box.sagernet.org/configuration/dns/server/local/)、[sing-box FakeIP](https://sing-box.sagernet.org/configuration/dns/server/fakeip/)、[sing-box TUN](https://sing-box.sagernet.org/configuration/inbound/tun/)、[Apple APNs network requirements](https://support.apple.com/102266)、[sing-box URLTest](https://sing-box.sagernet.org/configuration/outbound/urltest/) |
